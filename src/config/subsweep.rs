...
impl Subsweep {
    ...
    pub fn start_point(&self, config: &RadarConfig) -> i32 {
        unsafe { acc_config_subsweep_start_point_get(config.inner.as_ptr(), self.index) }
    }
    ...
    pub fn num_points(&self, config: &RadarConfig) -> u16 {
        unsafe { acc_config_subsweep_num_points_get(config.inner.as_ptr(), self.index) }
    }
    ...
    pub fn step_length(&self, config: &RadarConfig) -> u16 {
        unsafe { acc_config_subsweep_step_length_get(config.inner.as_ptr(), self.index) }
    }
    ...
    pub fn set_hwaas(&self, config: &mut RadarConfig, hwaas: Hwaas) {
        unsafe { acc_config_subsweep_hwaas_set(config.inner.as_ptr(), hwaas.into(), self.index) };
    }
    ...
    pub fn set_receiver_gain(&self, config: &mut RadarConfig, gain: u8) {
        unsafe { acc_config_subsweep_receiver_gain_set(config.inner.as_ptr(), gain, self.index) };
    }

    /// Gets receiver gain for subsweep
    pub fn receiver_gain(&self, config: &RadarConfig) -> u8 {
        unsafe { acc_config_subsweep_receiver_gain_get(config.inner.as_ptr(), self.index) }
    }
    ...
    pub fn set_transmitter_enabled(&self, config: &mut RadarConfig, enable: bool) {
        unsafe { acc_config_subsweep_enable_tx_set(config.inner.as_ptr(), enable, self.index) };
    }

    /// Gets transmitter enabled for subsweep
    pub fn is_transmitter_enabled(&self, config: &RadarConfig) -> bool {
        unsafe { acc_config_subsweep_enable_tx_get(config.inner.as_ptr(), self.index) }
    }
    ...
    pub fn set_prf(&self, config: &mut RadarConfig, prf: PulseRepetitionFrequency) {
        unsafe { acc_config_subsweep_prf_set(config.inner.as_ptr(), prf.into(), self.index) };
    }

    /// Gets Pulse Repetition Frequency for subsweep
    pub fn prf(&self, config: &RadarConfig) -> PulseRepetitionFrequency {
        let prf_val = unsafe { acc_config_subsweep_prf_get(config.inner.as_ptr(), self.index) };
    ...
    pub fn is_phase_enhancement_enabled(&self, config: &RadarConfig) -> bool {
        unsafe { acc_config_subsweep_phase_enhancement_get(config.inner.as_ptr(), self.index) }
    }
    ...
    pub fn is_loopback_enabled(&self, config: &RadarConfig) -> bool {
        unsafe { acc_config_subsweep_enable_loopback_get(config.inner.as_ptr(), self.index) }
    }
}
